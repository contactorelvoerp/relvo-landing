...
export const loginGradientFragmentShader = `#version 300 es
...
void main() {
  ...
  // ── BLOB A: top-center, above viewport ──
  if (u_blobCount > 0.5) {
    vec2 anchor = vec2(0.50, -0.30);
    vec2 drift = DRIFT(t, 0.0, 0.5, 0.15)
               + ORBIT_WOBBLE(t, 1.0, 0.06 * sc);
  ...
  // ── BLOB B: bottom-center, below viewport ──
  if (u_blobCount > 1.5) {
    vec2 anchor = vec2(0.50, 0.90);
    vec2 drift = DRIFT(t, 5.24, 5.74, 0.15)
               + ORBIT_WOBBLE(t, 5.3, 0.06 * sc);
    vec2 pos = anchor + drift;
    float d = length(screenPos - pos)
            + SHAPE_WOBBLE(screenPos, pos, t, 0.012 * sc);
    shape += smoothstep(blobSz, 0.0, d);
  ...
  // ── BLOB C: (unused) ──
  if (u_blobCount > 2.5) {
    vec2 anchor = vec2(0.50, 0.50);
    vec2 drift = DRIFT(t, 4.2, 4.7, 0.10);
  ...
  if (u_blobCount > 3.5) {
    vec2 anchor = vec2(0.50, 0.50);
    vec2 drift = DRIFT(t, 6.3, 6.8, 0.10);
    float d = length(screenPos - anchor - drift);
    shape += smoothstep(blobSz, 0.0, d);
  }
  ...
  if (u_blobCount > 4.5) {
    vec2 anchor = vec2(0.50, 0.50);
    vec2 drift = DRIFT(t, 8.4, 8.9, 0.10);
    float d = length(screenPos - anchor - drift);
    shape += smoothstep(blobSz, 0.0, d);
  }
  ...
  if (u_blobCount > 5.5) {
    vec2 anchor = vec2(0.50, 0.50);
    vec2 drift = DRIFT(t, 10.5, 11.0, 0.10);
    float d = length(screenPos - anchor - drift);
    shape += smoothstep(blobSz, 0.0, d);
  }
